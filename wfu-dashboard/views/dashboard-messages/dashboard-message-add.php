...
if (!class_exists('ACF')) {
	...
	//render the add form
	echo '<h1 class="wp-heading-inline">Add New Message</h1>';
	$aSites[0] = 'Global';
...
echo '<form id="dashboardMessageAddForm" name="dashboardMessageAddForm" action="' . esc_url( admin_url('admin-post.php') ) . '" method="POST">';
 	echo '<input type="hidden" name="action" value="dashboardMessageAddForm">';
  foreach ($aFields as $aFields => $aData){
    ...
    if ($aData['name'] == 'scope') {
    	...
    	echo '<textarea name="' . $aData['name'] . '" required="required"></textarea>';
    } else {
    	echo '<input id="' . $aData['name'] . '" name="' . $aData['name'] . '" type="text" required="required"><br/>';
    }
  }
  echo "<br/><br/><button class='button button-primary button-large' type='submit'>Add Message</button>";
echo '</form>';
